...
def save_best_model(x_train,y_train,params):
    make_gridCh=make_grid_search(x_train,y_train,grid_params=params,n_cv=2)
    best_model=make_gridCh.best_model
    best_params=make_gridCh.best_params
    make_gridCh.show_results()
    # Saving the trained model (sérialiser un modèle =Enregistrer le modèle et sa configuration:)
    # Sauvegarder le modèle et sa configuration
    model_path = project_root+"/models/best_model.pth"
    config_path = project_root+"/models/best_model_config.pth"
    torch.save(best_model.model.state_dict(), model_path)
    torch.save(best_params, config_path)
