...
def preprocess_data(input_df,filename):
    " preprocessing du dataframe firstname "
    columns=input_df.columns
    Trs_df=input_df.drop(['subject_line'],axis=1)
    ...
    df_w=df_w.rename(columns={"prediction":"Input_text"})
    df_w.to_csv(project_root+"/data/Clean_Data_before_tokenized"+filename+".csv")
    ...
    print( filename," : ", df_w.shape)
    Final_input=Final_input.input_ids
    Final_input.to_csv(project_root+"/data/df_after_tokenizeation"+filename+".csv")
    print("well done")
    return  Final_input
...
"""
def upload_preproces_in_online_Bucket(file_name_in_local,bucket_name=bucket_name,object_path_in_s3):
        # Initialisation du client boto3 pour MinIO
    file_path=project_root+"/data/"+file_name_in_local
...
# Envoi du fichier
    try:
        ...
        print(f"Fichier '{file_path}' envoyé avec succès au bucket '{bucket_name}' sous le nom '{object_path_in_s3}'.")
